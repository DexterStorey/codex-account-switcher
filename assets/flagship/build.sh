...
#   left        = the real tokmax dashboard (fixture, no daemon)
...
# final size (the tokmax fixture does not repaint on resize).
set -u
theme="${THEME:-dark}"
now="${TOKMAX_NOW:-1784133720000}"
...
# Right column at 46% (tokmax keeps the wider left), then split it in half.
tmux split-window -h -t fs:0.0 -l 46%
tmux split-window -v -t fs:0.1 -l 52%

tmux send-keys -t fs:0.0 \
  "clear; TOKMAX_FIXTURE=oneHot TOKMAX_THEME=$theme TOKMAX_NOW=$now TZ=America/Los_Angeles $bun run $repo/src/index.ts" Enter
